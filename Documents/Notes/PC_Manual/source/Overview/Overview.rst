...
更多精细化的dotfiles控制查看可以查看个人reSt笔记Git.rst

3. 软件包的安装
==================

刚刚配置好了dotfiles但是配置有了，软件没有，就需要安装软件

4. Sphinx 笔记
================

这个是来帮助你写笔记文档的，因为你通过安装dotfiles把笔记也全部拷贝下来了，所以在~/Documents/Notes/里有很多笔记，
...
命令                中文说明
===============     ============================================================================
paru <目标>         交互式搜索并安装 <目标> 软件包。
paru                paru -Syu 的别名（用于全盘更新系统）。
paru -S <目标>      安装指定的软件包（支持官方源和 AUR）。
paru -Sua           仅升级 AUR 软件包。
paru -Qua           显示可用的 AUR 更新列表。
paru -G <目标>      下载 <目标> 的 PKGBUILD 及相关文件到当前目录。
paru -Gp <目标>     打印（显示）<目标> 的 PKGBUILD 内容（用于审查代码）。
paru -Gc <目标>     打印 <目标> 的 AUR 评论信息。
paru --gendb        生成用于跟踪 \*-git 类型软件包的开发数据库。仅在初次使用 paru 时需要运行一次
paru -Bi .          构建并安装当前目录下的 PKGBUILD（用于本地编译安装）。
===============     ============================================================================
...
这是最常用的命令，等同于 pacman -Syu 但包含 AUR 包。
...
如果你发现 \*-git 版本的软件包无法正确检测更新，运行一次以下命令生成数据库：
